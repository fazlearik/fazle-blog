...
class BlogPost(db.Model):
    ...
    child_comment= relationship("Comment",back_populates="parent_post")
...
class Comment(db.Model):
    ...
    parent_post= relationship("BlogPost",back_populates="child_comment")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            ...
            flash("password does not exists")
            return redirect(url_for("login"))
        else:
            login_user(user)
            return redirect(url_for("get_all_posts"))
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            ...
            return redirect(url_for("login"))
        new_comment=Comment(text=form.comment_text.data,author=current_user,parent_post=requested_post)
